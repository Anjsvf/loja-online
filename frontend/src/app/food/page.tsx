        ...
        const fetchFoodItems = async () => {
            try {
                ...
                if (response.ok) {
                    ...
                    console.error("Erro ao buscar os itens de alimento.");
                }
            } catch (error) {
    ...
    return (
        <div className="food-page max-w-6xl mx-auto p-6">
            ...
            <section className="mb-10">
                <h2 className="text-2xl font-semibold text-gray-700 mb-4">Arroz</h2>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                    {renderItemsByName('Arroz').map(item => (
            ...
            <section className="mb-10">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                    {renderItemsByName('Açucar').map(item => (
                        <ProductItem key={item.id} product={item} />
                    ))}
                </div>
            </section>
            ...
            <section className="mb-10">
                <h2 className="text-2xl font-semibold text-gray-700 mb-4">Oléos </h2>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-6">
                    {renderItemsByName('Óleo').map(item => (
                        <ProductItem key={item.id} product={item} />
                    ))}
                </div>
            </section>
