...
export default function SupplemtsPage() {
  ...
  useEffect(() => {
    const fetchPerishableItems = async () => {
      try {
        ...
        if (!response.ok) {
          console.error("Erro ao buscar os itens de  suplemetos");
          return;
        }
        ...
        if (Array.isArray(items)) {
          const filteredItems = items.filter(item => item.category === "Suplementos");
  ...
  return (
    <div className="food-page max-w-6xl mx-auto p-6">
      <h1 className="text-3xl font-bold text-gray-800 mb-8">Suplemetos</h1>
      ...
      <section className="mb-10">
        <h2 className="text-2xl font-semibold text-gray-700 mb-4">Proteínas</h2>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
