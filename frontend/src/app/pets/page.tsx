    ...
    const fetchPerishableItems = async () => {
      try {
        ...
        if (Array.isArray(items)) {
          const filteredItems = items.filter(item => item.category === "Pets");
    ...
    <div className="food-page max-w-6xl mx-auto p-6">
      ...
      <section className="mb-10">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          {renderItemsByName("Adulto").map(item => (
      ...
      <section className="mb-10">
        <h2 className="text-2xl font-semibold text-gray-700 mb-4">Cães filhotes</h2>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          {renderItemsByName("Filhote").map(item => (
            <ProductItem key={item.id} product={item} />
          ))}
        </div>
      </section>
